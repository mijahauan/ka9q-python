...
Wire format (matches radiod's decode_socket() expectations):
    address(4 bytes, network order) + port(2 bytes, big-endian) = 6 bytes for IPv4
"""
...
class TestEncodeSocket:
    ...
    def test_encode_ipv4_address_default_port(self):
        ...
        assert bytes_written == 8  # type(1) + length(1) + data(6)
        assert buf[0] == StatusType.OUTPUT_DATA_DEST_SOCKET  # type
        assert buf[1] == 6  # length

        # Decode the socket structure: address(4) + port(2)
        addr_bytes = buf[2:6]
        port = struct.unpack('>H', buf[6:8])[0]
    ...
    def test_roundtrip_decode(self):
        ...
        # Skip type and length bytes, decode the 6-byte socket data
        socket_data = bytes(buf[2:])
        result = decode_socket(socket_data, 6)
